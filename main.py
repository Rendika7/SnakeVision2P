...
def run_scripts():
    ...
    controller_path = os.path.join(base_dir, "game-control-using-object-tracking-multithreaded-2players.py")
    game_path = os.path.join(base_dir, "SnakeGame2Player.py")

    # Putar musik latar
    play_music()

    # Jalankan game & controller secara paralel
    controller_proc = subprocess.Popen(["python", controller_path])
    time.sleep(1)  # Delay agar kamera siap
    game_proc = subprocess.Popen(["python", game_path])

    print("🎵 Musik dimulai • 🎮 Game & kontroler dijalankan")
